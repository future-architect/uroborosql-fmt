...
switch (platform) {
  ...
  case 'win32':
    ...
    break
  case 'darwin':
    localFileExisted = existsSync(join(__dirname, 'uroborosql-fmt-napi.darwin-universal.node'))
    try {
      if (localFileExisted) {
        nativeBinding = require('./uroborosql-fmt-napi.darwin-universal.node')
      } else {
        nativeBinding = require('uroborosql-fmt-napi-darwin-universal')
      }
      break
    } catch {}
    switch (arch) {
...
const { runLanguageServer } = nativeBinding
