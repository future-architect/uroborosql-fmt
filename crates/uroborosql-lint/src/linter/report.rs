use crate::{
    catalog::{input, resolution, AnalysisStatus, Resolution, ResolutionUnknown, UnknownReason},
    Diagnostic, SqlSpan,
};
    ...
    fn from(statement: &resolution::StatementResult) -> Self {
        Self {
            ...
            exclusion: statement.exclusion.map(|reason| match reason {
                input::Exclusion::SessionChange => CatalogExclusion::SessionChange,
                input::Exclusion::UnsupportedSyntax => CatalogExclusion::UnsupportedSyntax,
                input::Exclusion::UnsupportedIdentifier => CatalogExclusion::UnsupportedIdentifier,
                input::Exclusion::TemporarySchema => CatalogExclusion::TemporarySchema,
            }),
